...
def place_order():
    ...
    # Update address based on marker position
    if marker_position != st.session_state.map_center:
        geolocator = Nominatim(user_agent="pasto_verde_app")
        try:
            location = geolocator.reverse(f"{marker_position[0]}, {marker_position[1]}")
            if location:
                st.session_state.search_result = location.address
                st.success(f"Ubicación actualizada: {location.address}")
            else:
                st.warning("No se pudo encontrar la dirección para la ubicación seleccionada.")
        except Exception as e:
            st.error(f"Error al obtener la dirección: {str(e)}")
    ...
    full_address = f"{specific_address}, {st.session_state.search_result or ''}"
...
if 'marker_position' not in st.session_state:
    st.session_state.marker_position = [14.0818, -87.2068]  # Default to Tegucigalpa

map_with_marker(st.session_state.marker_position)

# Handle the marker position update
if st.session_state.get('marker_moved'):
    st.session_state.marker_position = [
        st.session_state.get('marker_lat'),
        st.session_state.get('marker_lng')
    ]
...
<script>
    if (window.Streamlit) {
        ...
        Streamlit.receiveMessageFromComponent = function(data) {
            if (data.moved) {
                Streamlit.setComponentValue({
                    marker_moved: true,
                    marker_lat: data.lat,
...
# Place this at the end of your main Streamlit script
if st.session_state.get('marker_moved'):
    handle_marker_moved(st.session_state.marker_lat, st.session_state.marker_lng)
def user_manual():
