...
def increment_and_get_count(session):
  # Increment the count
  visitor_record = session.query(VisitorCount).first()
  if visitor_record is None:
      visitor_record = VisitorCount(count=1)
      session.add(visitor_record)
  else:
      visitor_record.count += 1
  session.commit()
  return visitor_record.count
